    ...
    <div className="flex justify-start  space-x-2">
      ...
      {mediaFiles.slice(0, 3).map((file, index) => (
        <div key={index} className="relative">
          <img
            ...
            alt="uploaded"
            className="w-10 h-10 object-cover rounded-md border border-primary"
          />
          {/* Remove Button */}
          <button
            type="button"
            ...
            className="absolute -top-2 -right-2 bg-dark-red text-primary rounded-full p-1 w-6 h-6 flex items-center justify-center text-xs"
          >
            &times;
          </button>
        </div>
      ...
      {mediaFiles.length < 3 && (
        <label htmlFor="media-upload" className="cursor-pointer">
          <svg
            className="w-10 h-10 bg-primary border-light-gray rounded-md p-1"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            xmlns="http://www.w3.org/2000/svg"
