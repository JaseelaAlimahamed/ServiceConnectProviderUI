...
const BookingAppointment = ({ title, viewLink, day,time, userName, image, status, dateFrom ,dateTo, bookingTimeFrom = "07:00 AM", bookingTimeTo = "11:30 PM" }) => {
  return (
    <div className="flex max-w-lg mx-auto bg-white shadow-lg rounded-xl overflow-hidden h-44 relative">
      ...
      <div className="p-4 flex-1">
        <div className="flex justify-between items-center mb-2">
          ...
          <p className="text-sm md:text-xs lg:text-xs text-gray-500">{day},{time}</p>
        </div>
        <div className={`flex items-center ${status === 'Rescheduled' ? "mt-2" : "mt-6"}`}>
          <div className="flex space-x-2">
            ...
            :
            <div className="w-12 mr-3 h-12 bg-customPurple rounded-full object-cover"></div>
            }
            </div>
          <div>
